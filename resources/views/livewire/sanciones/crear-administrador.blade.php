<div>
    ...
    <div
        ...
        <div class="space-y-4">
            ...
            {{-- Campeonato --}}
            <div>
                <label class="block mb-1 font-bold dark:text-white">Campeonato</label>
                <select wire:model.live="buscarCampeonato"
                    ...
                    @foreach($campeonatos as $opcion)
                    <option value="{{ $opcion->id }}">{{ ucfirst($opcion->nombre) }}</option>
                    @endforeach
                </select>
        ...
        <div class="space-y-4 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-400">
            ...
            <div>
                ...
                <select wire:model.live="tipo_medida"
                    class="cursor-pointer w-full rounded-lg border border-gray-500 text-sm p-2.5">
            ...
            <div>
                <label class="block mb-1 text-sm font-semibold dark:text-white">
                    Cantidad de fechas a cumplir
                </label>
            ...
            <div>
                <label class="block mb-1 text-sm font-semibold dark:text-white">
                    Inhabilitado hasta el día
                </label>
                <input type="date" wire:model="fecha_fin"
                    class="bg-gray-50 border border-gray-500 text-gray-900 text-sm rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:text-white">

                <div class="grid grid-cols-2 gap-2 mt-3">
                    <button type="button" wire:click="setTiempo(1, 'year')" class="btn-tiempo">+1 Año</button>
                    <button type="button" wire:click="setTiempo(2, 'year')" class="btn-tiempo">+2 Años</button>
                    <button type="button" wire:click="setTiempo(6, 'month')" class="btn-tiempo">+6 Meses</button>
                    <button type="button" wire:click="setTiempo(3, 'month')" class="btn-tiempo">+3 Meses</button>
                </div>
            </div>
            @endif
        </div>
        ...
        <div class="md:col-span-2">
            <label class="block m-2 dark:text-white font-bold">
                4. Resolución del Tribunal (Observaciones)
            </label>
            <textarea wire:model="observacion" rows="3"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-500 dark:bg-gray-700 dark:text-white"
                placeholder="Detalle los motivos del fallo aquí..."></textarea>
        </div>

        {{-- BOTONES (FULL WIDTH) --}}
        <div class="md:col-span-2 flex flex-col md:flex-row gap-4 items-center mt-6 mb-20">
            <!-- botones sin cambios -->
        </div>
    </div>
