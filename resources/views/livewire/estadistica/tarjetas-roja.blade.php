<div class="space-y-4">
    ...
    <div
        ...
        <div class="w-full sm:w-1/2">
            <input type="text" wire:model.debounce.500ms="buscarRoja"
                placeholder="Buscar jugador por nombre o documento"
                class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring focus:border-red-400" />
        </div>
    </div>
    ...
    <div class="hidden sm:block bg-white dark:bg-gray-800 shadow rounded-lg overflow-x-auto">
        <table class="w-full text-sm sm:text-base text-left text-gray-700 dark:text-gray-100">
            <thead class="text-xs sm:text-sm uppercase bg-gray-700 dark:bg-gray-700 text-red-400">
                <tr>
                    <th class="px-4 py-2">Documento</th>
                    <th class="px-4 py-2">Jugador</th>
                    <th class="px-4 py-2">Equipo</th>
                    <th class="px-4 py-2 text-center">Rojas</th>
                    <th class="px-4 py-2">Partido</th>
                </tr>
            </thead>
            <tbody>
                @forelse($rojas as $jug)
                <tr class="border-b dark:border-gray-600">
                    ...
                    <td class="px-4 py-2 text-center">{{ $jug->tarjeta_roja }}</td>
                    <td class="px-4 py-2">
                ...
                <tr>
                    <td colspan="5" class="px-4 py-6 text-center text-gray-500">
                        @if ($buscarRoja)
                        No se encontraron coincidencias para <strong>"{{ $buscarRoja }}"</strong>.
                        @else
                        No hay tarjetas rojas registradas en este campeonato.
                        @endif
                    </td>
                </tr>
                @endforelse
    ...
    <div class="sm:hidden space-y-4">
        @forelse($rojas as $jug)
        <div class="bg-red-50 dark:bg-gray-800 rounded-lg shadow p-4 text-sm text-gray-800 dark:text-gray-100">
            ...
            <div><span class="font-semibold">Tarjetas Rojas:</span> {{ $jug->tarjeta_roja }}</div>
