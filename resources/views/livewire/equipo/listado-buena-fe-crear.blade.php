<div class="relative overflow-x-auto shadow-md sm:rounded-lg dark:bg-gray-600">
    <div class="bg-blue-900 text-white p-2 shadow-md rounded flex justify-between items-center relative z-10">
        <h2 class="font-semibold text-xl text-gray-100 leading-tight">
            {{ __('Agregar jugador a Listado de Buena Fe') }}
        </h2>
    ...
    <div class="grid grid-cols-3 gap-4 mb-4 bg-gray-500 m-2 text-white p-2 rounded">
        <div>
            <label>Equipo</label>
            <select wire:model="equipoSeleccionado" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
        ...
        <div>
            ...
            <input type="text" wire:model.live.debounce.500ms="buscar" placeholder="Nombre, apellido o DNI" x-data
                x-on:focus-input.window="setTimeout(() => $el.focus(), 50)" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                    focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
                    dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white
                    dark:focus:ring-blue-500 dark:focus:border-blue-500" />
        </div>
    ...
    <div class="p-2">
        ...
        <div class="mb-4 bg-gray-800 border border-gray-300 text-gray-100 text-sm rounded-lg
                    focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
                    dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white
                    dark:focus:ring-blue-500 dark:focus:border-blue-500">
            @foreach($jugadores as $jug)
            @if(!collect($jugadoresSeleccionados)->pluck('id')->contains($jug->id))
            <div class="flex justify-between items-center border-b py-1">
                <span>{{ strtoupper($jug->apellido) }}, {{ strtoupper($jug->nombre) }} - DNI ( {{ $jug->documento }}
                    )</span>
                <button wire:click="agregarJugador({{ $jug->id }})" class="bg-green-700 text-white hover:bg-green-500 rounded-full font-semibold
                                hover:text-black px-2 py-2 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <table class="w-full border-collapse border border-gray-400 mb-4">
            ...
            <tbody>
                ...
                <tr class="bg-gray-500 border border-gray-300 text-gray-100">
                    ...
                    <td class="border px-2">{{ strtoupper($jug['apellido']) }}, {{ strtoupper($jug['nombre']) }}</td>
                    <td class="border px-2">{{ strtoupper($jug['documento']) }}</td>
                    <td class="border px-2 text-center">
                        <button wire:click="quitarJugador({{ $i }})"
                            class="bg-red-500 text-white px-2 m-1 py-2 hover:bg-red-400 rounded-full cursor-pointer">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="size-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
